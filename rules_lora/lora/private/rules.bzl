...
# ============================================================
# _lora_local_config — emit the local-backend training config.
# ============================================================
#
# Reads the recipe + base providers and writes `<name>.local.json`
# (hyperparams + base id/rev/family). The //lora:defs.bzl macro feeds it
# to the `<name>_local` py_binary (runtime/local_runner:local_train),
# which renders the torchtune config + drives the venv at run time. No
# shell of ours anywhere: the config is structured data and the runner is
# a real py_binary (replaces the former generated bash + local_runner.sh).
#
# Why a rule (not the macro): providers aren't readable in macros, so we
# need a rule to extract `recipe.rank`, `recipe.alpha`, etc.
...
def _lora_local_config_impl(ctx):
    ...
    out = ctx.actions.declare_file(ctx.label.name + ".local.json")
    config = {
        "name": ctx.attr.adapter_name,
        "base_id": base_info.id,
        "base_revision": base_info.revision,
        "family": ctx.attr.family,
        "rank": recipe_info.rank,
        "alpha": recipe_info.alpha,
        "target_modules": recipe_info.target_modules,
        ...
        # Runfiles-relative path of the validated dataset JSONL; the runner
        # resolves it via the python runfiles lib (the dataset itself is wired
        # into the py_binary's data by the macro).
        "dataset": _runfiles_path(dataset_info.jsonl, ctx),
    }
    ctx.actions.write(output = out, content = json.indent(json.encode(config), indent = "  ") + "\n")
    return [DefaultInfo(files = depset([out]))]

lora_local_config = rule(
    implementation = _lora_local_config_impl,
    attrs = {
...
def _lora_merge_impl(ctx):
    base_info = ctx.attr.base[LoraBaseModelInfo]
    out = ctx.actions.declare_file(ctx.label.name + ".merge.json")
    config = {
        "adapter_dir": ctx.attr.adapter_dir,
        "base_id": base_info.id,
        "base_revision": base_info.revision,
        "out_dir": ctx.attr.out_dir,
        "push_repo": ctx.attr.push_repo,
