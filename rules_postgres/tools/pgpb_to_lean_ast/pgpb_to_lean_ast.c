...
static void emit_raw_stmt(FILE *o, const PgQuery__RawStmt *rs,
                          int typed, int skip_other_bytes) {
    /* In default mode, emit `Pg.Query.RawStmt` with `stmt : ByteArray`.
     * In --typed mode, emit `Pg.Query.Top.TopRawStmt` with `stmt : TopStmt`.
     *
     * `skip_other_bytes` (--skip-other-bytes) replaces unrecognized
     * stmts' opaque payload with `_root_.ByteArray.empty`. The
     * catalog fold ignores `.other` Nodes entirely, so the bytes
     * contribute zero to the resulting Snapshot — but elaborating
     * a 5000-element UInt8 literal (a PLpgSQL function body) burns
     * heartbeats. For diff tests on real schemas, this flag makes
     * elaboration feasible without changing fold semantics. */
    fputs("    { stmt := ", o);
    if (typed) {
...
/* Emit a single TopParseResult/ParseResult value combining stmts
 * from one or more input ParseResults. The combined output mirrors
 * what `pgpb_to_snapshot.c` does when fed multiple inputs: stmts
 * are concatenated in input-file order, then within each file in
 * stmt order. The `version` field carries the first input's
 * version (they should all agree). */
static void emit_parse_result(FILE *o,
                              PgQuery__ParseResult **prs, size_t n_prs,
                              const char *module_name,
                              int typed, int skip_other_bytes) {
    fprintf(o, "/- Auto-generated by @rules_postgres//tools:pgpb_to_lean_ast.\n"
               "   DO NOT EDIT BY HAND \xe2\x80\x94 regenerate from .pgpb input(s). -/\n\n");
    /* Lean rule: imports first, then set_option, then namespace. */
    if (typed) {
        fprintf(o, "import Pg.Query.Top\n\n");
    } else {
        fprintf(o, "import Pg.Query.Generated\n\n");
    }
    /* Elaborating very long TopRawStmt lists (savvi initial_schema
     * = 1384 stmts) overruns Lean's default recursion / heartbeat
     * limits. Bumping these once at the top is cheaper than per-
     * consumer overrides. maxRecDepth scales with list length;
     * 65k handles inputs up to ~30k stmts. maxHeartbeats is a
     * separate elaboration-time budget. */
    fprintf(o, "set_option maxRecDepth 65536\n");
    fprintf(o, "set_option maxHeartbeats 64000000\n\n");
    if (typed) {
    ...
    int32_t version = (n_prs > 0) ? prs[0]->version : 0;
    fprintf(o, "  version := %d\n", version);

    /* Count total stmts across all inputs. */
    size_t total = 0;
    for (size_t i = 0; i < n_prs; i++) total += prs[i]->n_stmts;
    ...
    if (total == 0) {
        ...
        for (size_t i = 0; i < n_prs; i++) {
            for (size_t j = 0; j < prs[i]->n_stmts; j++) {
                emit_raw_stmt(o, prs[i]->stmts[j], typed, skip_other_bytes);
                emitted++;
                if (emitted < total) fputs(",\n", o);
                else fputs("\n", o);
            }
        }
        fprintf(o, "  ]\n");
...
int main(int argc, char **argv) {
    ...
    int typed = 0;
    int skip_other_bytes = 0;
    /* Inputs are collected in argv order. Caller supplies them in
     * the order the resulting catalog fold should see (e.g.
     * lexicographic by filename for savvi's `010_*.sql`,
     * `020_*.sql`, …). */
    const char **inputs = (const char **)calloc(argc, sizeof(char *));
    size_t n_inputs = 0;
    int argi = 1;
    while (argi < argc) {
        if (strcmp(argv[argi], "--module") == 0 && argi + 1 < argc) {
            ...
            typed = 1;
        } else if (strcmp(argv[argi], "--skip-other-bytes") == 0) {
            skip_other_bytes = 1;
        } else if (argv[argi][0] != '-') {
            inputs[n_inputs++] = argv[argi];
        } else {
    ...
    if (!module || !output || n_inputs == 0) {
        fprintf(stderr, "usage: pgpb_to_lean_ast --module <NAME> "
                        "--output <FILE> [--typed] <input.pgpb> [<input.pgpb> ...]\n");
        free(inputs);
        return 1;
    }

    PgQuery__ParseResult **prs =
        (PgQuery__ParseResult **)calloc(n_inputs, sizeof(PgQuery__ParseResult *));
    for (size_t i = 0; i < n_inputs; i++) {
        size_t n = 0;
        unsigned char *buf = read_file(inputs[i], &n);
        if (!buf) { free(inputs); free(prs); return 2; }
        prs[i] = pg_query__parse_result__unpack(NULL, n, buf);
        free(buf);
        if (!prs[i]) {
            fprintf(stderr, "pgpb_to_lean_ast: failed to unpack %s\n", inputs[i]);
            for (size_t j = 0; j < i; j++)
                pg_query__parse_result__free_unpacked(prs[j], NULL);
            free(inputs); free(prs);
            return 2;
        }
    }
    ...
    if (!out) {
        ...
        for (size_t i = 0; i < n_inputs; i++)
            pg_query__parse_result__free_unpacked(prs[i], NULL);
        free(inputs); free(prs);
        return 2;
    }

    emit_parse_result(out, prs, n_inputs, module, typed, skip_other_bytes);
    fclose(out);

    size_t total = 0;
    for (size_t i = 0; i < n_inputs; i++) total += prs[i]->n_stmts;
    fprintf(stderr, "  %zu file(s), %zu stmts \xe2\x86\x92 %s\n",
            n_inputs, total, output);
    for (size_t i = 0; i < n_inputs; i++)
        pg_query__parse_result__free_unpacked(prs[i], NULL);
    free(inputs); free(prs);
    return 0;
